 ...
 *   new information — the agent incorporates it in its reply (AC-23).
...
test.describe('Kontekst czatu — odpowiedź na follow-up i obsługa pytań off-topic', () => {
  ...
  test('AC-22,AC-23 agent uwzględnia nowe informacje podane w wiadomości follow-up', async ({ page }) => {
    ...
    // AC-22: agent has full context (form data + image analysis + prior messages)
    // AC-23: agent incorporates new info (scratch, one week of use)
    const reply = await chatPage.getLastAgentBubbleText();
    expect(reply.length).toBeGreaterThan(50);

    // The reply should reference the new information provided
    // TODO(3.2): confirm exact Polish phrasing from the agent
    await expect(
      page.locator('.message-bubble.agent, [data-role="assistant"]').last()
    ).toContainText(/zadrapanie|[uż]ywany|stan/i, { timeout: 60_000 }); // TODO(3.2): confirm selector
  });

  test('AC-24 wskaźnik pisania jest widoczny podczas generowania odpowiedzi', async ({ page }) => {
    const chatPage = await submitAndGetChat(page);

    // Send a message and check that typing indicator appears before response
    const messagePromise = chatPage.sendMessage('Jak długo trwa procedura zwrotu?');

    // Typing indicator should be visible immediately after sending
    // (it appears while the agent streams)
    // TODO(3.2): confirm typing indicator appears — depends on streaming implementation timing
    // await expect(chatPage.typingIndicator).toBeVisible({ timeout: 10_000 });
  ...
  test('AC-27 pytanie off-topic jest grzecznie odrzucane i przekierowane', async ({ page }) => {
    ...
    await expect(
      ...
      /nie mog[ęe] pom[oó]c|poza zakresem|dotyczy tylko|reklamacja|zwrot|serwis|przeznaczony/i,
      { timeout: 60_000 }
    ); // TODO(3.2): confirm selector
  });
  ...
  test('AC-27 pytanie o ogólne porady prawne jest przekierowane do celu asystenta', async ({ page }) => {
    ...
    await expect(
      ...
      /nie mog[ęe] pom[oó]c|poza zakresem|dotyczy.*reklamacj|dotyczy.*zwrot/i,
      { timeout: 60_000 }
    ); // TODO(3.2): confirm selector
  });
